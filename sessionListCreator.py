            ...
            for row in rows:
                ...
                if 'Lecture' in data:
                    for ea in data['Lecture']:
                        count = 1
                        random_key = random.choice(list(lecturer_preferences.keys()))
                        sess = Session(lecturer_preferences[random_key],'Lecture',cCode,data['count'],count)
                        sess.setPriority(4)
                ...
                if 'Lab' in data:
                    for ea in data['Lab']:
                        count = 1
                        random_key = random.choice(list(lecturer_preferences.keys()))
                        sess = Session(lecturer_preferences[random_key],'Lab',cCode,round((int(data['count']) / len(data['Tutorial'])* 1.3)),count)
                        sess.setPriority(2)
                        sess.setTimeSpan(random.randint(1, 2))
                        allSessions[cCode] = sess
                        count +=1
                ...
                if 'Tutorial' in data:
                    for ea in data['Tutorial']:
                        count = 1
                        random_key = random.choice(list(lecturer_preferences.keys()))
                        sess = Session(lecturer_preferences[random_key],'Tutorial',cCode,round((int(data['count']) / len(data['Tutorial'])* 1.3)),count)
                        sess.setPriority(1)
                        sess.setTimeSpan(1)
                        allSessions[cCode] = sess
                        count +=1
                ...
                if 'Seminar' in data:
                    for ea in data['Seminar']:
                        count = 1
                        random_key = random.choice(list(lecturer_preferences.keys()))
                        sess = Session(lecturer_preferences[random_key],'Seminar',cCode,data['count'],count)
                        sess.setPriority(3)
                        sess.setTimeSpan(1)
                        allSessions[cCode] = sess
                        count +=1
...
def makeRegistrationData():
    regis ={}
    file_name = 'Mock_Data_Updated.csv'
    allsess = list(getAllSessions().values())
    ListAllSessions = [ele.name for ele in allsess]
    ...
    #    Create a CSV reader object
        ...
        for row in rows:
            id = int(row[0].strip())
            sessName = ListAllSessions[random.randint(0,list_length -1)]
